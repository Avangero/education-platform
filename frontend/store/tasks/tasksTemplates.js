...
export default {
    ...
    mutations: {
        ...
        UPDATE_TASK_TEMPLATE(state, { value }) {
            state.list.find(
                (taskTemplate) => taskTemplate.id === Number(value.id)
            ).id = value.id;
            state.list.find(
                (taskTemplate) => taskTemplate.id === Number(value.id)
            ).title = value.title;
    ...
    actions: {
        async getTasksTemplates({ commit }) {
            ...
            return await get('/api/mentor/tasks-templates/')
                .then((data) => {
        ...
        async createTaskTemplate({ commit }) {
            commit('SET_LOADING', true);
            return await post('/api/mentor/tasks-templates/create')
                .then((data) => {
        ...
        async saveTaskTemplate({ commit, getters }, { taskTemplateId }) {
            return await post(
                `/api/mentor/tasks-templates/${taskTemplateId}`,
                getters.getListItemById(taskTemplateId)
            )
                .then((data) => {
